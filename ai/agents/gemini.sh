...
# Install Gemini CLI and write local-model env vars for Ollama routing.
...
setup_gemini() {
    ...
    if [ -z "$src_cfg" ]; then
        ...
        src_cfg="${SETTINGS_BASE}/ai/profiles/$mac_model/gemini/settings.json"
    fi
    ...
    # Write env file for local-model mode via Ollama proxy
    local env_file
    mkdir -p "$HOME/.profile.d"
    env_file="$HOME/.profile.d/_gemini"

    cat > "$env_file" << 'EOF'
# Gemini CLI — route to local Ollama proxy (comment out to use real Gemini API)
# export GOOGLE_GEMINI_BASE_URL="http://localhost:11434"
