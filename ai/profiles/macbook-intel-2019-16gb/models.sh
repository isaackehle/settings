...
#
# Intel 2019 MBP specific notes (2026-06-13):
#   - Architecture: x86_64 (9th-gen Intel Core i7/i9). No Apple Silicon unified
#     memory — system RAM is discrete DDR4 separate from dGPU VRAM.
#   - dGPU: AMD Radeon Pro 4GB (560X) or 8GB (5500M/555X). Metal/ROCm support
#     in Ollama is partial on Intel macs; CPU is the reliable path.
#   - iGPU (Intel UHD 630): not used for LLM inference.
#   - Recommended Ollama env: OLLAMA_NUM_GPU=0 (CPU-only) unless you verify
#     Metal offload helps your specific model. See /Library/LaunchAgents/com.kehle.ollama.plist
#     for the override pattern.
#   - Network mode: this box lives in the garage. Run ollama with
#     OLLAMA_HOST=0.0.0.0:11434 and reach it from the main M5 Max over Tailscale.
#   - Use the same lightweight stack as macbook-m1-16gb; the 16GB constraint is
#     identical. What differs is the execution substrate, not the model set.
...
elif [ -n "${SETTINGS_BASE:-}" ] && [ -f "${SETTINGS_BASE}/ai/profiles/macbook-intel-2019-16gb/models.json" ]; then
    _MODELS_JSON="${SETTINGS_BASE}/ai/profiles/macbook-intel-2019-16gb/models.json"
else
