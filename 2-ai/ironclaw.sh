...
setup_ironclaw() {
    ...
    src_cfg="${SETTINGS_BASE}/2-ai/profiles/${MACHINE_PROFILE}/ironclaw/.env"
    src_yaml="${SETTINGS_BASE}/2-ai/profiles/${MACHINE_PROFILE}/ironclaw/ironclaw.yaml"
    if [ ! -f "$src_cfg" ]; then
        src_cfg="${SETTINGS_BASE}/2-ai/profiles/default/ironclaw/.env"
    fi
    if [ ! -f "$src_yaml" ]; then
        src_yaml="${SETTINGS_BASE}/2-ai/profiles/default/ironclaw/ironclaw.yaml"
    fi
