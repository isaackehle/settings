...
PROFILE_DIR="${SETTINGS_BASE}/2-ai/profiles/${MACHINE_PROFILE}"
MODELS_FILE="${PROFILE_DIR}/models.sh"
RUNTIME_FILE="${SETTINGS_BASE}/2-ai/runtime.sh"
PATHS_FILE="${SETTINGS_BASE}/2-ai/paths.sh"
VALIDATE_PROFILE_FILE="${SETTINGS_BASE}/2-ai/validate-profile.sh"
...
usage() {
    cat <<'EOF'
Usage:
  2-ai/llama-cpp.sh run <role|alias> [--prompt TEXT] [--ctx N] [--temp F] [--extra ...]
  2-ai/llama-cpp.sh serve <role|alias> [--port N] [--ctx N] [--parallel N] [--extra ...]
  2-ai/llama-cpp.sh bench <role|alias> [--ctx N] [--prompt-tokens N] [--gen-tokens N] [--extra ...]
  2-ai/llama-cpp.sh inspect <role|alias>

Examples:
  2-ai/llama-cpp.sh run coder --prompt "Explain this stack trace"
  2-ai/llama-cpp.sh serve general --port 8012
  2-ai/llama-cpp.sh bench heavy --gen-tokens 256
EOF
}
