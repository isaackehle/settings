...
deploy_configs() {
  ...
  {
    echo "# Required Ollama models for $(basename "${_profdir}")"
    echo "# Generated by setup_ai.sh deploy_configs — edit to add/remove models"
    echo "# One model tag per line. Lines starting with # are ignored."
    echo ""

    # Collect from all sources
    local _all_models=()

    # OLLAMA_MODELS (skip :cloud entries)
    for _m in "${OLLAMA_MODELS[@]}"; do
      [[ "$_m" != *":cloud" ]] && _all_models+=("$_m")
    done
    ...
    if declare -p OPENCODE_AGENTS &>/dev/null 2>&1; then
      for _k in "${!OPENCODE_AGENTS[@]}"; do
        _all_models+=("${OPENCODE_AGENTS[$_k]}")
      done
    fi

    # CONTINUE_ROLES
    if declare -p CONTINUE_ROLES &>/dev/null 2>&1; then
      for _k in "${!CONTINUE_ROLES[@]}"; do
        _all_models+=("${CONTINUE_ROLES[$_k]}")
      done
    fi

    # CLAUDE_CODE
    if declare -p CLAUDE_CODE &>/dev/null 2>&1; then
      for _k in "${!CLAUDE_CODE[@]}"; do
        _all_models+=("${CLAUDE_CODE[$_k]}")
      done
    fi
